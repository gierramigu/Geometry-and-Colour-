...
private:
	...
	std::vector<unsigned int> m_indices; //how to connect the position vertices 

	// GPU-side data
	GLuint m_vao = 0; // vao = vertex array objects 
	GLuint m_vbo_pos = 0; // vbo = vertex buffer objects
	GLuint m_vbo_norm = 0;
	GLuint m_ibo = 0;
...
public:
	...
	void setup() {
		if (m_vao == 0) {
			// generate buffers
			// creating a buffer to send the info from the cpu to the gpu
			...
			//this allows the buffer to be bound to the currently bound gl array buffer
